...
public class MaskedInputFormatterTextWatcher implements TextWatcher {
    ...
    public MaskedInputFormatterTextWatcher(MaskCharsMap maskCharsMap, char placeholder) {
    ...
    public SpannableStringBuilder getFormattedMask() {
        ...
        for (int i = 0; i < mask.length(); i++) {
            char ch = mask.charAt(i);
            boolean prevMaskChar = i > 0 && builder.charAt(i - 1) == placeholder;
            MaskChar maskChar = maskCharsArr[i];
            if (maskChar != null) {
                builder.append(placeholder);
                Object span = maskChar.getSpanForPlaceholder();
                if (span != null) {
            ...
            if (source == placeholder) {
                ...
                if (editablePlacesBeforeCursor >= 0) {
                    if (editablePlacesBeforeCursor == 0) {
                        if (newCharsCount != 1
                                || counter == 0
                                || unmaskedValue.charAt(counter - 1) != placeholder) {
                            newSelectionStart = i;
                        }
                    } else {
                        newSelectionStart = i;
            ...
            if (!TextUtils.isEmpty(charsFromLastGroup)) {
                ...
                if (newCharsCount > 0) {
                    ...
                    if (groupText.length() < group.length) {
                        while (groupText.length() < group.length) {
                            groupText.insert(newCharsLocalPos, placeholder);
                        }
                    } else if (newCharsStart == end && !deletionOfMultipleChars) {
                        groupText.replace(newCharsLocalPos - 1, newCharsLocalPos, String.valueOf(placeholder));
                    }
                }
                newCharsWasHandled = true;
                ...
                if (!deletionOfMultipleChars) {
                    ...
                    if (nextGroup == null || (nextStart == newCharsStart && nextEnd - nextStart == nextGroup.length)) {
                        groupText.replace(group.length - 1, group.length, String.valueOf(placeholder));
                    }
                }
            }
            while (groupText.length() < group.length) {
                groupText.insert(groupText.length(), placeholder);
            }
            lastGroupEnd = end;
    ...
    public String getUnmaskedValue(String s) {
        ...
        for (int i = 0; i < min; i++) {
            char ch = s.charAt(i);
            if (ch != mask.charAt(i) && ch != placeholder) {
                builder.append(ch);
            }
        }
